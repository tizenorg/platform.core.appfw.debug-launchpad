...
int send_app_dead_signal(int dead_pid)
{
	DBusMessage *message;
...
int send_app_launch_signal(int launch_pid)
{
	DBusMessage *message;
...
static int __sigchild_action(void *data)
{
	...
	if (dead_pid == get_gdbserver_pid())
		dead_pid = get_gdbserver_app_pid();
...
void debug_launchpad_sigchld(struct signalfd_siginfo *info)
{
	int status;
	pid_t child_pid;
	pid_t child_pgid;
...
int signal_get_sigchld_fd(void)
{
	sigset_t mask;
	int sfd;
...
int signal_unblock_sigchld(void)
{
