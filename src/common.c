...
static char *__appinfo_get_app_path(appinfo_t *appinfo)
{
	...
	if (path_len == 0) {
		...
		temp_app_path = (char *)malloc(sizeof(char) * (path_len + 1));
...
appinfo_t *appinfo_create(bundle *kb)
{
	...
	if (__apppinfo_get_app_path(appinfo) == NULL) {
...
static char **__add_arg(bundle * kb, char **argv, int *margc, const char *key)
{
	...
	if (str_array) {
		if (strncmp(key, DLP_K_DEBUG_ARG, strlen(key)) == 0
			...
			new_argv = (char **)realloc((len + 2) * sizeof(char *));
			if (new_argv == NULL) {
...
char **create_argc_argv(bundle *kb, int *margc, const char *app_path)
{
	...
	if (bundle_get_type(kb, AUL_K_SDK) & BUNDLE_TYPE_ARRAY)
		...
		str = bundle_get_str(kb, AUL_K_SDK);
		if (str) {
			str_array = &str;
			len = 1;
		}
