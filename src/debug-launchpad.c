		...
		appinfo_t *appinfo, bundle *kb)
{
	...
	const char *value;
	 ...
	 if (value && strncmp(value, SDK_ATTACH, strlen(SDK_ATTACH)) == 0) {
		 ...
		 if (__set_access(appid, appinfo->pkg_type, app_path) < 0) {
			 _D("Failed to set privileges - "
				"check your package's credential");
			 return -1;
		 }
	 }
		...
		bundle *kb, appinfo_t *appinfo)
{
	...
	pid = fork();
	if (pid == 0) {
		if (_gdbserver_is_running())
			_adjust_process_capability(CAPABILITY_SET_INHERITABLE);
...
static gboolean __handle_launch_event(gpointer data)
{
	...
	pid = __start_process(pkg_name, app_path, kb, appinfo);
