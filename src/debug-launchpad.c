		...
		appinfo_t *appinfo, bundle *kb)
{
	...
	const char *value;
	 ...
	 __preexec_run(appinfo->pkg_type, appid, app_path);

	 value = bundel_get_val(kb, AUL_K_SDK);
	 if (value && strncmp(value, ATTACH, strlen(ATTACH)) == 0) {
		 /* SET PRIVILEGES */
		 _D("pkg_name: %s / pkg_type: %s / app_path: %s",
				 pkg_name, menu_info->pkg_type, app_path);
		 if (__set_access(pkg_name, menu_info->pkg_type, app_path) < 0) {
			 _D("Failed to set privileges - "
...
static int __prepare_fork(bundle *kb, const char *appid)
{
	...
	prepare_debug_tool(kb, appid, str_array, len);

	return 0;
}

static int __redirect_stdfds(int pid)
{
	char path[PATH_MAX];
	int fd;
	int ret = 0;
	...
	if (fd < 0) {
		_E("Failed to open caller stdout: %d", pid);
		ret = -1;
	} else {
		dup2(fd, 1);
		close(fd);
	...
	fd = open(path, O_WRONLY);
	if (fd < 0) {
		_E("Failed to open caller stderr: %d", pid);
		ret = -1;
	} else {
		dup2(fd, 2);
		close(fd);
...
static void __real_launch(const char *app_path, bundle *kb)
{
	...
	app_argv = create_argc_argv(kb, &app_argc, app_path);
		...
		bundle *kb, appinfo_t *appinfo)
{
	...
	if (pid == 0) {
		if (gdbserver_is_running())
			adjust_process_capability(CAPABILITY_SET_INHERITABLE);
		...
		if (iter_fd = 3; iter_fd <= max_fd; iter_fd++)
			close(iter_fd);
		...
		if (__redirect_stdfds(__get_caller_pid(kb)))
			_E("Failed to redirect standard file descriptors");
	...
	SECURE_LOD("==> real launch pid: %d %s", pid, app_path);
...
static gboolean __handle_sigchld(gpointer data)
{
	...
	do {
		...
		debug_launchpad_sigchld(&siginfo);
	} while (s > 0);
...
static gboolean __handle_launch_event(gpointer data)
{
	...
	const char *debug_appid = NULL;
	int pid = -1;
	int clifd = -1;
	struct ucred cr;

	pkt = app_recv_raw(fd, &clifd, &cr);
	if (pkt == NULL) {
		_E("packet is NULL");
		return TRUE;
	}
	...
	appinfo = appinfo_create(kb);
	if (appinfo == NULL) {
		_E("_appinfo_create() is failed.");
		goto end;
	}

	apppath = appinfo->app_path;
	if (app_path == NULL) {
		_E("app_path is NULL");
		goto end;
	}
	...
	SECURE_LOGD("exec: %s", appinfo->app_path);
	SECURE_LOGD("debug appid: %s", appinfo->debug_appid);

	modify_bundle(kb, cr.pid, appinfo, pkg->cmd);
	pid = __start_process(pkg_name, app_path, debug_appid, kb, menu_info);

end:
	__send_result_to_caller(clifd, pid, app_path);

	if (pid > 0)
		send_app_launch_signal(pid);
	if (appinfo)
		appinfo_free(appinfo);
	if (kb)
		bundle_free(kb);
	if (pkt)
		free(pkt);
	if (get_valgrind_option())
		wait_for_valgrind_output();
...
static int __debug_launchpad_init(int argc, char **argv)
{
	int fd;

	/* create debug-launchpad socket */
	fd = create_server_sock();
	if (fd < 0) {
...
static int __before_loop(int argc, char **argv)
{
	...
	if (__debug_launchpad_init(argc, argv) < 0) {
